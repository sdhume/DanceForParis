...
class TwitterProxy {
	 ...
	 */
	public function __construct($oauth_access_token, $oauth_access_token_secret, $consumer_key, $consumer_secret, $user_id, $hashtag, $count = 5) {
		$this->config = array_merge($this->config, compact('oauth_access_token', 'oauth_access_token_secret', 'consumer_key', 'consumer_secret', 'user_id', 'hashtag', 'count'));
		$this->whitelist['search/tweets.json?user_id=' . $this->config['user_id'] . '&q=%23' . $this->config['hashtag'] . '&count=' . $this->config['count']] = true;
	}
